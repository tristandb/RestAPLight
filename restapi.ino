...
static uint8_t myip[4] = { 192,168,0,105};
  ...
  if(pin != NULL){
    if(value != NULL){
      ...
      String string = "";
        ...
        unsigned int selectedPin = string.toInt();
        unsigned int minimal = 50;
#ifdef DEBUG
      Serial.println(selectedPin);
#endif
if(selectedPin >= minimal){
          RemoteTransmitter::sendCode(9, selectedPin, period, 3);
      ...
      bfill.emit_p(PSTR(
        ...
        "Access-Control-Allow-Origin: *"
        "\r\n"
        "<link rel=\"shortcut icon\" href=\"data:image/x-icon;,\" type=\"image/x-icon\"> {\"$S\":\"$S\"}"), pin, outValue);
      return bfill.position();     
    }
  } else {
